...
import com.henau.mall.product.service.CategoryBrandRelationService;
...
public class CategoryServiceImpl extends ServiceImpl<CategoryDao, CategoryEntity> implements CategoryService {
    ...
    /**
    * 每一个需要缓存的数据我们都来指定需要放到哪个名字的缓存。【缓存的分区（按照业务类型分）】
    * @Cacheable({"category"})
    ...
    *   缓存的value的值，默认使用jdk序列化机制，将系列化后的数据存到Redis
    *   默认TTL时间 -1
    *
    * 自定义操作
    ...
    */
    @Cacheable(value = {"category"}, key = "#root.method.name", sync = true)   //代表当前方法的结果需要缓存，如果缓存中有，方法不用调用。如果缓存中没有，就会调用方法，最后将方法的结果放入缓存
    @Override
    ...
    //TODO 产生堆外内存溢出：outOfDorectMemoryError
    //1)、springboot2.0以后默认使用lettuce作为操作Redis的客户端。他使用netty进行网络通信。
    //2)、lettuce的bug导致netty堆外内存溢出 -Xmx300m:netty如果没有指定堆外内存，默认使用-Xmx300m
    //可以通过-Dio.netty.maxDirecMemory进行设置
    //解决方案：不能使用-Dio.netty.maxDirecMemory只去调大堆外内存。
...
//    @Override
    public Map<String, List<Catelog2Vo>> getCatalogJson2() {
        //给缓存中放json字符串，哪出的json字符串，还用逆转为能用的对象类型【序列化与方序列化】
