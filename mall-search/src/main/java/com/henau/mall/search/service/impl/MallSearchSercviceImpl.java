...
import com.henau.mall.search.feign.ProductFeignService;
import com.henau.mall.search.service.MallSearchSercvice;
import com.henau.mall.search.vo.AttrResponseVo;
import com.henau.mall.search.vo.BrandVo;
...
@Service
public class MallSearchSercviceImpl implements MallSearchSercvice {
    ...
    /**
     * 准备检索请求
     * #模糊匹配，过滤（按照属性，分类，品牌，价格期间，库存），排序，分页，高亮，聚合分析
     * @return
     */
    private SearchRequest buildSearchRequest(SearchParam param) {

        SearchSourceBuilder sourceBuilder  = new SearchSourceBuilder(); //构件DSL语句的

        /**
         * 查询：模糊匹配，过滤（按照属性，分类，价格区间，品牌，库存）。
         */
        //1、构件bool-query
    ...
    private SearchResult buildSearchResult(SearchResponse response,SearchParam param) {
        ...
        if(param.getAttrs()!=null && param.getAttrs().size()>0){
            List<SearchResult.NavVo> collect = param.getAttrs().stream().map(attr -> {
                ...
                //2.取消了面包屑以后，我们要跳转到哪个地方,将请求地址的URL里面的当前置空
                //拿到所有的查询条件，去掉当前
                String replace = replaceQueryString(param, attr, "attrs");
                navVo.setLink("http://search.henaumall.com/list.html?"+replace);
                return navVo;
            }).collect(Collectors.toList());
